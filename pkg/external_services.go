...
/*
RetrieveOne2 Retrieve External Services Configuration
...
*/
func (a *ExternalServicesApiService) RetrieveOne2(ctx _context.Context, servicename string) (models.ExternalServicesPropertiesData, *_nethttp.Response, error) {
	var (
		...
		fileName     string
		fileBytes    []byte
		returnValue  models.ExternalServicesPropertiesData
	)

	// create path and map variables
	path := a.client.cfg.BasePath + "/externalservice/{servicename}"
	path = strings.Replace(path, "{"+"servicename"+"}", _neturl.QueryEscape(parameterToString(servicename, "")), -1)
...
/*
UpdateExternalServiceProperties Update External Service
Updates the external Service Configuration for a Service Name.  Example:   externalservice/S3
 * @param ctx _context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 * @param servicename servicename
 * @param putExternalServiceRequest
*/
func (a *ExternalServicesApiService) UpdateExternalServiceProperties(ctx _context.Context, servicename string, putExternalServiceRequest models.PutExternalServiceRequest) (*_nethttp.Response, error) {
	var (
	...
	// body params
	postBody = &putExternalServiceRequest
	if ctx != nil {
		// API Key Authentication
		if auth, ok := ctx.Value(ContextAPIKey).(APIKey); ok {
			var key string
